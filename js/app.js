...
// Build 76: old coaching 73/72 reconstruction removed. Coaching is now static HTML.
// Keep the previously approved Founders wording, without changing any assessment controls.
(() => {
  ...
  const alignAnchor = () => {
    ...
    coaching.style.setProperty('--ra76-anchor-offset', `${height + 12}px`);
  };
  alignAnchor();
  if (header && 'ResizeObserver' in window) new ResizeObserver(alignAnchor).observe(header);
  else window.addEventListener('resize', alignAnchor, {passive:true});

  // Graceful fallback: a blocked/missing decorative image reveals the live-text layout.
  const artwork = coaching.querySelector('.ra76-approved-art');
  const checkArtwork = () => {
    const failed = window.matchMedia('(min-width:1200px)').matches && artwork.complete && artwork.naturalWidth === 0;
    coaching.classList.toggle('is-art-unavailable', failed);
  };
  artwork.addEventListener('error', checkArtwork);
